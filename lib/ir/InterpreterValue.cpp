...
VInteger VInteger::operator+(const VInteger &other) const {
  ...
  return VInteger(newValue, isSigned, bitWidth);
}
VInteger VInteger::operator-(const VInteger &other) const {
  assert(isSameType(other) && "Both integers must have the same type");
  std::uint64_t newValue = value - other.value;
...
VInteger VInteger::operator/(const VInteger &other) const {
  if (other.value == 0) {
    llvm::report_fatal_error("Division by zero");
  }
  INT_OPERATE(/);
}
VInteger VInteger::operator%(const VInteger &other) const {
...
VInteger VInteger::operator-() const {
  if (isSigned) {
    ...
    case 8:
      return VInteger(
          static_cast<std::int8_t>(-static_cast<std::int8_t>(value)), isSigned,
          bitWidth);
    case 16:
      return VInteger(
          ...
          isSigned, bitWidth);
    case 32:
      return VInteger(-static_cast<std::int32_t>(value), isSigned, bitWidth);
    case 64:
      return VInteger(-static_cast<std::int64_t>(value), isSigned, bitWidth);
    default:
      llvm_unreachable("Unsupported integer bit width");
    }
  } else {
...
VFloat VFloat::operator-(const VFloat &other) const { FLOAT_OPERATE(-); }
VFloat VFloat::operator*(const VFloat &other) const { FLOAT_OPERATE(*); }
VFloat VFloat::operator/(const VFloat &other) const {
  if (other.isZero()) {
    llvm::report_fatal_error("Division by zero");
  }
  FLOAT_OPERATE(/);
...
VFloat VFloat::fromAPFloat(llvm::APFloat apFloat) {
