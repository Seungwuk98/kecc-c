...
int fibonacci(int n) {
  ...
  for (i = 1; i < n; ++i) {
    ...
    // CHECK-NEXT:   %b6:i1:i32 = add %b6:i0:i32 1:i32
    // CHECK-NEXT:   %b6:i2:unit = store %b6:i1:i32 %l1:i32*
    // CHECK-NEXT:   j b4()
  }

  // CHECK-LABEL: block b7:
  // CHECK-NEXT:   %b7:i0:i32 = load %l3:i32*
  // CHECK-NEXT:   ret %b7:i0:i32
  return t2;
}
// CHECK-NEXT: }
