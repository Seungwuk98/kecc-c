...
int fibonacci(int n) {
  ...
  do {
    ...
    // CHECK-NEXT:   %b4:i1:i32 = load %l3:i32*
    // CHECK-NEXT:   %b4:i2:i32 = add %b4:i0:i32 %b4:i1:i32
    // CHECK-NEXT:   %b4:i3:unit = store %b4:i2:i32 %l4:i32*
    t1 = t2;
    // CHECK-NEXT:   %b4:i4:i32 = load %l3:i32*
    // CHECK-NEXT:   %b4:i5:unit = store %b4:i4:i32 %l2:i32*
    t2 = next_term;
    // CHECK-NEXT:   %b4:i6:i32 = load %l4:i32*
    // CHECK-NEXT:   %b4:i7:unit = store %b4:i6:i32 %l3:i32*
    ++i;
    // CHECK-NEXT:   %b4:i8:i32 = load %l1:i32*
    // CHECK-NEXT:   %b4:i9:i32 = add %b4:i8:i32 1:i32
    // CHECK-NEXT:   %b4:i10:unit = store %b4:i9:i32 %l1:i32*
    // CHECK-NEXT:   j b5()
  } while (i < n);
  // CHECK-LABEL: block b5:
  // CHECK-NEXT:   %b5:i0:i32 = load %l1:i32*
  // CHECK-NEXT:   %b5:i1:i32 = load %l0:i32*
